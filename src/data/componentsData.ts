        ...
        <motion.span
            ...
            variants={{
                ...
                loop: {
                    ...
                    // We can also add a slight bounce or color shift if desired, but user specifically asked for italic transform.
                    scaleY: [1, 0.9, 1], // Subtle squash during wave
                    transition: {
                        duration: 1,
                        repeat: Infinity,
                        repeatDelay: 2,
...
// With custom styles
<ExpandableStrips className="h-[600px] w-full" />`,
        fullCode: `"use client";
...
export function ExpandableStrips({ className = "", style }: { className?: string; style?: React.CSSProperties }) {
    ...
    return (
        ...
        >
            ...
            <div 
                ref={containerRef}
                className="flex w-full md:max-w-6xl h-[500px] px-[35vw] md:px-4 gap-4 items-center overflow-x-auto md:overflow-visible pb-8 md:pb-0 scrollbar-hide"
...
function Strip({
    ...
    return (
        <motion.div
            ...
            onMouseLeave={onLeave}
            data-id={item.id}
            className={\`relative h-[85%] overflow-hidden cursor-pointer \${shouldExpand ? "min-w-[280px] z-10 shadow-2xl" : "min-w-[100px] z-0"} md:min-w-0\`}
            initial={{ flex: 1, opacity: 0, scale: 0 }}
            animate={{
                flex: shouldExpand ? 3.5 : 1,
                y: verticalOffset,
                opacity: 1,
                scale: 1,
            }}
            transition={{
                flex: { duration: 0.6, ease: [0.32, 0.72, 0, 1] },
